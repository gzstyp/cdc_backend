...
public class UserController{
    ...
    /**授权撤权*/
    @PreAuthorize("hasAuthority('user_row_editAudit')")
    @PostMapping("/editAudit")
    public void editAudit(final HttpServletRequest request,final HttpServletResponse response){
        ToolClient.responseJson(userService.editAudit(ToolClient.getFormData(request)),response);
    }
    ...
    public void queryTreeArea(final HttpServletRequest request,final HttpServletResponse response){
        ToolClient.responseJson(userService.queryTreeArea(ToolClient.getFormData(request)).replaceAll("\"false\"","false").replaceAll("\"true\"","true"),response);
    }
