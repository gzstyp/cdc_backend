...
import com.fwtai.web.CrowdCategoryDao;
...
*/
@Service
public class CrowdCategoryService{

    private final Logger logger = LoggerFactory.getLogger(getClass());

    @Resource
    private CrowdCategoryDao crowdcategoryDao;
    ...
    public String edit(final HttpServletRequest request){
        ...
        return ToolClient.executeRows(crowdcategoryDao.edit(formData));
    }
    ...
    public String queryById(final PageFormData pageFormData){
        ...
        if(validate != null)return validate;
        return ToolClient.queryJson(crowdcategoryDao.queryById(pageFormData.getString(p_id)));
    }
    ...
    public String delById(final PageFormData formData){
        ...
        final String exist_key = crowdcategoryDao.queryExistById(kid);
        if(exist_key == null){
            return ToolClient.createJson(ConfigFile.code199,"删除失败,数据已不存在");
        }
        return ToolClient.executeRows(crowdcategoryDao.delById(kid));
    }
    ...
    public String delByKeys(final PageFormData formData){
        ...
        return ToolClient.executeRows(crowdcategoryDao.delByKeys(lists),"操作成功","数据已不存在,刷新重试");
    }
    ...
    public String listData(PageFormData formData){
        ...
        final HashMap<String,Object> map = crowdcategoryDao.listData(formData);
